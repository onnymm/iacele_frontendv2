...
const InputText: React.FC<IACele.UI.Input.Text.Params> = ({
    ...
    // Inicialización de funciones para uso en enfoque del campo
    const { isFocused, onFocus, onBlur } = useInputText();
        ...
        <div className={`${isFocused ? 'border-primary-500' : 'border-gray-500/20 hover:border-primary-500/50'} relative h-10 border-2 rounded-lg transition-colors duration-100`}>
            ...
            {Icon &&
                <div className="top-0 absolute flex flex-row items-center size-full pointer-events-none">
                    <Icon className={`${isFocused ? 'text-primary-500' : 'text-gray-500'} ml-2 min-w-5 h-5 transition-colors duration-100`} />
                </div>
            }
        </div>
    );
...
const useInputText = (): IACele.Hook.UI.InputText => {
    ...
    // Inicialización de función onFocus
    ...
    // Inicialización de función onBlur
