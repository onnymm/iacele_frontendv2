...
const GroupRouteButton: React.FC<IACele.Application._Sidebar.UI.RouteGroupButton> = ({
    ...
    return (
        <button onClick={onClick} className={`${isActiveLocation ? "shadow-md active:shadow-md backdrop-saturate-115 backdrop-brightness-125" : ""} flex flex-row items-center gap-2 sm:hover:shadow-md active:shadow-md sm:hover:backdrop-saturate-115 sm:hover:backdrop-brightness-125 active:backdrop-saturate-115 active:backdrop-brightness-125 px-2 rounded-md w-full h-8 transition`}>
            ...
            {/* Nombre del grupo */}
            <span className="flex-grow text-sm text-start">{name}</span>

            {/* Lista de rutas, en caso de haberla */}
            {typeof routes === 'object' &&
                <div className="flex justify-center items-center min-w-9 sm:min-w-7 h-9 sm:h-7">
                    <ChevronDown className={`${isOpen ? 'rotate-180' : ''} duration-300 transition text-current size-[75%]`} />
                </div>
            }
